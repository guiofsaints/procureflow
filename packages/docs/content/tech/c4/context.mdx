...
graph TD
    %% Actors
    Employee[👤 Employee/Requester<br/>Internal user purchasing<br/>materials and services]
    Buyer[👤 Procurement Specialist<br/>Buyer managing catalog<br/>quality and approvals]
    Engineer[👤 Platform Engineer<br/>DevOps/SRE deploying<br/>and monitoring system]

    %% System Boundary
    ProcureFlow[🎯 ProcureFlow Web App<br/>Next.js 15 / React 19<br/>AI-native procurement platform]

    %% External Systems
    MongoDB[(🗄️ MongoDB Atlas M0<br/>Document database<br/>512 MB free tier)]
    OpenAI[🤖 OpenAI API<br/>GPT-3.5-turbo<br/>LLM for agent]
...
```
Employee → [HTTPS] → ProcureFlow (Catalog Search)
ProcureFlow → [MongoDB Wire] → MongoDB (Full-text Query)
MongoDB → [Results] → ProcureFlow
ProcureFlow → [HTTPS] → Employee (Catalog Items)
```

### 2. Agent Chat Flow

```
Employee → [HTTPS] → ProcureFlow (Agent Message)
ProcureFlow → [HTTPS REST] → OpenAI (LLM Inference)
OpenAI → [Function Call] → ProcureFlow (Tool: search_catalog)
ProcureFlow → [MongoDB Wire] → MongoDB (Execute Search)
MongoDB → [Results] → ProcureFlow
ProcureFlow → [Response] → Employee (Agent Message + Items)
```

### 3. Checkout Flow

```
Employee → [HTTPS] → ProcureFlow (Checkout Request)
ProcureFlow → [MongoDB Wire] → MongoDB (Create Purchase Request)
ProcureFlow → [MongoDB Wire] → MongoDB (Clear Cart)
MongoDB → [Success] → ProcureFlow
ProcureFlow → [HTTPS] → Employee (PR Confirmation)
```
...
✅ Catalog search with MongoDB full-text indexing  
✅ Item registration with duplicate detection  
✅ Shopping cart management (add, update, remove, analytics)  
✅ Purchase request submission and history tracking  
✅ AI agent with 8 integrated tools  
✅ User authentication and session management

### Out of Scope (v1.0)

❌ Approval workflows (auto-submit to "Submitted")  
❌ Real ERP integration (PRs logged to MongoDB only)  
❌ Email notifications (no status change emails)  
❌ Advanced analytics (no dashboards/reports)  
