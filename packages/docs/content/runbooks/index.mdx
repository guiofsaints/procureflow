...
| ---------------------------------------------------- | -------------------------------------------------------- | ---------------------- | --------------------------- | ---------- |
| **[Local Development](/runbooks/local-dev)**         | Set up local development environment with Docker Compose | Daily (new developers) | Developers                  | 🟢 Low     |
| **[Build and Deploy](/runbooks/build-and-deploy)**   | Execute deployment to GCP Cloud Run (dev/staging/prod)   | Weekly (releases)      | Platform Engineers          | 🟡 Medium  |
| **[Rollback](/runbooks/rollback)**                   | Rollback failed deployment using Cloud Run or Pulumi     | As-needed (incidents)  | Platform Engineers, On-Call | 🔴 High    |
| **[Autoscaling Check](/runbooks/autoscaling-check)** | Monitor and verify Cloud Run autoscaling behavior        | Weekly (ops review)    | Platform Engineers          | 🟢 Low     |
| **[Troubleshooting](/runbooks/troubleshooting)**     | Diagnose and resolve common failures (10 scenarios)      | As-needed (incidents)  | All Engineers, On-Call      | 🟡 Medium  |
...
2. **Metadata**: Duration, complexity, prerequisites
...
- **New to codebase?** → [Local Development](/runbooks/local-dev)
- **Build failing?** → [Troubleshooting](/runbooks/troubleshooting)

### For Platform Engineers

- **Deploying a release?** → [Build and Deploy](/runbooks/build-and-deploy)
- **Deployment failed?** → [Rollback](/runbooks/rollback)
- **Checking scaling?** → [Autoscaling Check](/runbooks/autoscaling-check)

### For On-Call Engineers

- **Incident response?** → [Troubleshooting](/runbooks/troubleshooting)
- **Need to rollback?** → [Rollback](/runbooks/rollback)
...
| ---------------------- | ----------------------------------------------------------------------- | --------- | -------------- |
| **Database Migration** | Execute schema migration with rollback plan                             | 🔴 High   | v1.2 (Q1 2026) |
| **Secrets Rotation**   | Rotate NEXTAUTH_SECRET, OPENAI_API_KEY, MongoDB credentials             | 🟡 Medium | v1.2 (Q1 2026) |
| **Incident Response**  | Incident management workflow (detect → mitigate → resolve → postmortem) | 🔴 High   | v1.3 (Q2 2026) |
| **Backup and Restore** | MongoDB backup to GCS, restore from backup                              | 🟡 Medium | v1.2 (Q1 2026) |
| **Load Testing**       | Execute k6 load tests, analyze results                                  | 🟢 Low    | v1.3 (Q2 2026) |
