...
- **Employee**: Search catalog → Add to cart → Checkout → Track purchase requests
- **Buyer**: Register new items → Review duplicate warnings → Monitor spending (future analytics)
- **Engineer**: Deploy infrastructure → Monitor health → Rollback on failures
...
**Failure Mode**: Database unavailable → Application returns 503 Service Unavailable for all endpoints requiring data access (catalog, cart, checkout, agent)
...
**Failure Mode**: API unavailable → Agent returns fallback message "I'm having trouble processing your request. Please try using the catalog page.", catalog/cart features remain functional
